...
export class EventService {
  ...
  baseUrl : string = 'http://localhost:3000/Sadmin/showNews';
  search_index: number;
  ...
  createEvent(event, token) {
    ...
    fData.append('content', event.content);
    fData.append('title', event.title);
    fData.append('specialist', localStorage.getItem('specialist'));
    console.log(event.image,event.content,event.title,localStorage.getItem('specialist'))
