...
@Component({
  selector: 'sb-add-fci-news',
  templateUrl: './add-fci-news.component.html',
  styleUrls: ['./add-fci-news.component.scss']
})
export class AddFciNewsComponent implements OnInit {
  images : any[] = [];
  msg = "";
  ...
  onMultipleSubmit(userData:any){
    if(userData.valid){
      ...
      this._auth.addFciNews(formData).subscribe(result=> console.log(result))
